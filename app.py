...
def login_required(view_function):
    ...
    protected_view.__name__ = view_function.__name__
...
def get_drive_service():
    ...
    if not file_id or not service_json:
        raise RuntimeError(
            "Faltan GOOGLE_DRIVE_FILE_ID o GOOGLE_SERVICE_ACCOUNT_JSON en Render."
        )
...
def download_excel_from_drive():
    ...
    request_file = service.files().get(fileId=file_id, fields="id,name,modifiedTime")
    metadata = request_file.execute()
