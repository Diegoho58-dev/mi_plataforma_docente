...
def create_empty_planning_blocks(buffer, selected_subjects, decisions=None, curriculum_buffer=None, selected_cleis=None, selected_themes=None, selected_objectives=None, selected_articulations=None):
...
def planning_examples(buffer, subject, clei, limit=5):
    ...
    buffer.seek(0)
    workbook = load_workbook(buffer, data_only=True, read_only=True)
    worksheet = workbook.get(subject)
...
def actualizar_planeacion():
    ...
    page={"options": options, "cleis": cleis, "topics_by_subject": {item["value"]: {clei: [] for clei in cleis} for item in options}, "selected": [], "selected_cleis": {}, "selected_themes": {}, "selected_objectives": {}, "selected_articulations": {}, "proposals": {}, "created": [], "already_exists": [], "error": None, "message": None, "is_friday": friday, "only_friday": PLANNING_ONLY_FRIDAY}
    try:
        ...
        if request.method == "POST":
            ...
            if not selected:
                ...
                updated_buffer, created, already_exists = create_empty_planning_blocks(buffer, selected, selected_cleis=selected_cleis, selected_themes=selected_themes, selected_objectives=selected_objectives, selected_articulations=selected_articulations)
                if created:
